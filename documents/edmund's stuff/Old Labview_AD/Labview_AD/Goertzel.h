...
public:
    ...
    Goerzel_result get_results(Goerzel& G)
    {
        ...
        gr.amp = sqrtf(real * real + imaginary * imaginary);
        gr.phase = atan2f(imaginary, real);
        if(gr.phase<0)
            gr.phase += 2*PI;
        gr.phase*=180/PI;
        G.initialize();
    ...
    float power_only_all(long* data)
    {
        Goerzel G;
        for(int i = 0; i < n; ++i)
            process(G, data[i]);
    ...
    float power_only(Goerzel& G)
    {
        float real = (G.q1*a + G.q2*c);
        float imaginary = (G.q1*b+G.q2*d);
...
{
public:
    divider_process(float Target_Freq, int Divider = 1) : process_data(Target_Freq, (CURRENT_SAMPLE_RATE/Divider) + .5), divider(Divider)
    {
