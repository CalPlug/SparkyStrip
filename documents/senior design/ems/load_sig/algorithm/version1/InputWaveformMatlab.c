...
// boolean
int boolean[NUMBER_OF_CHANNELS];
...
void read_input(int CH)
{
	...
	if(CH == 0)
		...
		fp[CH] = fopen("Input3.txt", "r");
	else if(CH == 4)
		fp[CH] = fopen("Input4.txt", "r");
	else
		fp[CH] = fopen("Input5.txt", "r");
...
{	
	// boolean returns 0 if it doesnt find the WaveForm type, and 1 if it does
	boolean[CH] = 0;
