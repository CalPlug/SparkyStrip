...
void draw()
{
  if(mouseclicked)
  {
    ...
    if(output[0].equals("1"))
    {
      ...
      image(tv, width/2+200, 350);
    }
    if(output[0].equals("2"))
    {
      fill(green);
      ellipse(width/2-80, 465, 15, 15);
      image(stb, width/2+200, 350);
    }
...
void mousePressed() 
{
  ...
  fill(black);
  textFont(fontA, 20);
  text("Rise Time: " + output[1] + "  sec", width/2+100, 570);
  text("Normalized Magnitude at 180 Hz: " + output[2], width/2+100, 620);
  text("Normalized Magnitude at 300 Hz: " + output[3], width/2+100, 670);
