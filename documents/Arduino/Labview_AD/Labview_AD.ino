...
void send_update_v(){
    float package[10] = { 0,0,v60.amplitude(),v60.phase(),v180.amplitude(),v180.phase(),v300.amplitude(),v300.phase(),v420.amplitude(),v420.phase()};
    if(Serial)
#ifdef HUMAN_READABLE
    {
        Serial.print("\nVoltage Results:");
...
void send_update_i(){
    float package[10] = {active_power,power_factor,i60.amplitude(),i60.phase(),i180.amplitude(),i180.phase(),i300.amplitude(),i300.phase(),i420.amplitude(),i420.phase()};
    if(Serial)
#ifdef HUMAN_READABLE
    {
        Serial.print("\nCurrent Results:");
...
#ifdef HUMAN_READABLE
    if(Serial)
        Serial.write("\nWatchdog triggered, resetting AD values...\n");
#endif
...
#endif //HUMAN_READABLE
    while(!tcp_connect(IP, PORTNO))  //loop until it connects
#ifdef HUMAN_READABLE
        if(Serial)
